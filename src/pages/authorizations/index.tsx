...
export default function AuthorizationPage() {
  ...
  async function authenticateUser(event: any) {
    ...
    //todo: переписать на рабочий сервер и добавление в базу данных
    try {
  ...
  async function registerNewUser(event: any) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        return <div>Поздравляю, пользователь зарегистрирован!</div>;
      } else {
        let error = new Error(response.statusText);
              ...
              >
                <TabPanel value="sighIn">
                  <Typography color="gray" className="mt-1 font-normal">
                    для сохранения результатов расчетов, пожалуйста, войдите в
                    личный кабинет
                  </Typography>
                ...
                <TabPanel value="register">
                  <Typography color="gray" className="mt-1 font-normal">
                    для сохранения результатов расчетов, пожалуйста,
                    зарегистрируйтесь
                  </Typography>
                  <form className="mt-8 mb-2">
                    <div className="mb-4 flex flex-col gap-6">
                    ...
                    <Button
                      ...
                      //todo: добавить уведомление о том, что пользователь зарегистрировался и отпарвить письмо на адрес
                    >
                      ЗАРЕГИСТРИРОВАТЬСЯ
                    </Button>
                    {userData.error && (
